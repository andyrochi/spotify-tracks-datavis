...
export const MultiSelectDropdown = (params) => {
    let config = {
      search: true,
      hideX: false,
      useStyles: true,
    ...
    document.querySelectorAll('select[multiple]').forEach((multiSelect) => {
        ...
        Array.from(multiSelect.options).map((option) => {
          let optionElement = newElement('div', { class: [option.selected ? 'checked' : '', option.text], srcElement: option });
        ...
        div.refresh = () => {
          ...
          if (multiSelect.selectedOptions?.length === 0) {
            ...
            allGenreOptionDiv.classList.toggle('checked');
            allGenreOptionDiv.querySelector('input').checked = true;
            allGenreOptionDiv.srcElement.selected = true;
            div.refresh();
            return;
            // div.appendChild(
            //   newElement('span', {
